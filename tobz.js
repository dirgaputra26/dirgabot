...
let dbcot = JSON.parse(fs.readFileSync('./lib/database/dbblacklist.json'))
...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6281414199896@c.us'
        const Admin1  = '6289650591381@c.us'
        ...
        function isReg(obj){
            if (obj === true){
                ...
                return tobz.reply(from, `*WAJIB GUNAKAN GUILD KALIAN SEKARANG,INGAT GUILD KALIAN SEKARANG!!JIKA TIDAK RESIKO BANNED* Kamu belum terdaftar sebagai MEMBER DIRGA BOT\nuntuk mendaftar kirim ${prefix}daftar |nama|umur|guild\n\ncontoh format: ${prefix}daftar |DIRGA|17|guild\n\ncukup gunakan nama depan/panggilan saja`, id) //if user is not registered
            }
        }
        ...
        function cekumur(obj){
            if (obj === true){
                ...
                return tobz.reply(from, `Kamu belum cukup umur untuk menggunakan BOT, min 16 tahun\n\nKamu bisa mendaftar ulang dengan cara donasi terlebih dahulu, bales ${prefix}donasi\nHubungi Owner : wa.me/628141419896`, id) //if user is not registered
            }
        }
        ...
        if (chats == 'Assalamualaikum'){
            tobz.reply(from, 'Waalaikumsalam wr wb.', id)
        }
        if (chats == `Daftar Borocial Jogja`){
            tobz.reply(from, 'Hai ini adalah pesan otomatis', id)
        }
        if (chats == `Daftar Borocial Jogja`){
            tobz.reply(from, 'Selamat Datang di Pendaftaran Borocial Jogja untuk melanjutkan ketik (menu jogja) ya kak thanks', id)
        }
        if (chats == `daftar borocial jogja`){
            tobz.reply(from, 'Hai ini adalah pesan otomatis', id)
        }
        ...
        if (chats == `menu jogja`){
            tobz.reply(from, '1.formulir jogja 1\n2.jumlah guild \n3.info\nketik perintah sesuai nomor!', id)
        }
        if (chats == `formulir jogja`){
            tobz.reply(from, '/jogja PROSEDUR PENDAFTARAN\n1. HUBUNGI ADMIN SESUAI TUJUAN\n2. ISI LALU KIRIM BIODATA KE ADMIN\n3. SS KD LIFETIME DAN JUMLAH MATCH (SQUAD)\n4. FOTO KK/KTP/KARTU PELAJAR (KIRIM SALAH SATU UNTUK MENGECEK DOMISILI & USIA)\n5. JANGAN SPAM\nNB : JIKA TIDAK SESUAI DENGAN DIATAS MAKA AKAN DI BLACKLIST\nMengisi biodata dengan format\nNama :\nNick :\nAsal :\nUmur :\nId :\nLv :\nGender :\nAsal Guild :\nAlasan Masuk Guild :\nAlasan Keluar Guild Lama :\nScreenshot Badge + Lifetime (Squad)', id)
        }
if (chats == 'assalamualaikum'){
            tobz.reply(from, 'Waalaikumsalam wr wb.', id)
        }
        if (chats == 'P'){
            tobz.reply(from, 'asu diem njing', id)
        }
        if (chats == 'p'){
            tobz.reply(from, 'asu diem njing', id)
        }
        ...
        if (chats == 'kontol'){
            tobz.sendPtt(from, './media/astag.mp3', id)
        }
        if (chats == 'Kontol') {
            tobz.sendPtt(from, './media/astag.mp3', id)
        }
        ...
        if (chats == 'bego'){
            tobzsendPtt(from, './media/astag.mp3', id)
        }
        if (chats == 'Bego'){
            tobz.sendPtt(from, './media/astag.mp3', id)
        }
        ...
        if (chats == 'fjebferf'){
            tobz.sendPtt(from, './media/septi.mp3', id)
        }		
        if (chats == 'e fcern v cebcen'){
            tobz.sendPtt(from, './media/septi.mp3', id)
        }		
        if (chats == 'jebfcjerbjvbjkervbjj'){
            tobz.sendPtt(from, './media/septi.mp3', id)
        }
        if (chats == 'Peraturan'){
            tobz.sendPtt(from, './media/AD ART BOROCIAL GAMERS (BRO).pdf', id)
        }
        if (chats == 'Afridza'){
            tobz.sendPtt(from, './media/afrid.mp3', id)
        }
        if (chats == 'afridza'){
            tobz.sendPtt(from, './media/afrid.mp3', id)
        }
        ...
        function isStickerMsg(id){
            ...
            for (let i of stickerspam){
                if(i.id === id){
                    if (i.msg >= 12) {
                        found === true 
                        tobz.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh BOT DIRGA', message.id).then(() => {
        ...
        function isBadwordMsg(id){
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 1) { // 12x
                        kasar === true 
                        tobz.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh dirga!', message.id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        }).then(() => {
                            const cus = id
                            var found = false
        ...
        if(!isCmd && isKasar && isGroupMsg && isBadword && !isGroupAdmins) { 
            ...
            if(isBadwordMsg(serial)) return
                ...
                if(body === '/unmute' && isMuted(chatId) == false){
                    if(isGroupMsg) {
                        if(!isOwner) return tobz.reply(from, 'maaf', id)
...
**NB : TOTAL 75 GUILD*`
                ...
                break
                case prefix+'level':
                if (!isLevelingOn) return tobz.reply(mess.levelnoton)
                const userLevel = getLevelingLevel(sender)
                const userXp = getLevelingXp(sender)
                if (userLevel === undefined && userXp === undefined) return tobz.reply(mess.levelnol)
			...
			case prefix+'leveling':
                ...
                if (args.length < 1) return tobz.reply('Boo :𝘃')
                if (args[0] === 'enable') {
                    if (isLevelingOn) return tobz.reply('*fitur level sudah aktif sebelum nya*')
                    _leveling.push(groupId)
                    fs.writeFileSync('./lib/leveling.json', JSON.stringify(_leveling))
                     tobz.reply(mess.levelon)
                } else if (args[0] === 'disable') {
                    _leveling.splice(groupId, 1)
                    fs.writeFileSync('./lib/leveling.json', JSON.stringify(_leveling))
                     tobz.reply(mess.leveloff)
                } else {
                   tobz.reply(' *Ketik perintah 1 untuk mengaktifkan, 0 untuk menonaktifkan* \n *Contoh: ${prefix}leveling 1*')
                }
            break
        case prefix+'aduanofficer':
            tobz.sendContact(chatId, `6288231226371@c.us`)
            tobz.reply(from, `Aduan officer chat nomor tersebut` , id)
            break
        case prefix+'banchat':
        ...
        case prefix+'stiker':
            ...
            tobz.reply(from, 'wa.me/628141419896@c.us https://www.instagram.com/dirgaap._ Owner BOT', id)
        ...
        case prefix+'ttp2':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#ttp2 [ Teks ]*, contoh *#ttp2 *`, id)
        ...
        case prefix+'blackpink':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#blackpink [ Teks ]*, contoh *#blackpink*`, id)
            tobz.reply(from, mess.wait, id)
        ...
        case prefix+'watear':
             if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isAdmin) return tobz.reply(from, 'Maaf, dalam perbaikan!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah */water [ Teks ]*, contoh /water Zfa*`, id)
            tobz.reply(from, mess.wait, id)
            const water = body.slice(23)
            if (water.length > 10) return tobz.reply(from, '*Teks Terlalu Panjang!*\n_Maksimal 10 huruf!_', id)
             await tobz.sendFileFromUrl(from, `https://api.vhtear.com/water_maker?text=${water}&apikey=${vhtearkey}`, 'thndr.jpg', '', id)
             break
        case prefix+'nulissekolah':
             if(isReg(obj)) return
            if(cekumur(cekage)) return
   
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah */nulissekolah [ Teks ]*, contoh /nulissekolah Zfa*`, id)
            tobz.reply(from, mess.wait, id)
            const nama = body.slice(14)
            const kelas = body.slice(14)
            const nulissekolah = body.slice(14)
            if (nulissekolah.length > 40000) return tobz.reply(from, '*Teks Terlalu Panjang!*\n_Maksimal 200 huruf!_', id)
             await tobz.sendFileFromUrl(from, `https://api.vhtear.com/magernulis?nama=${nama}&kelas=${kelas}&text=${nulissekolah}&tinta=`, 'nulis.jpg', '', id)
            break     
        case prefix+'pornhub':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Zfa| Dirga*`, id)
            ...
            if (argz.length >= 2) {
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Zfa|Dev DIRGA*`, id)
            }
            break
            case prefix+'googlemaker':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah */googlemaker [ |Teks1|Teks2|teks 3 ]*, `, id)
            argz = body.trim().split('|')
            if (argz.length >= 3) {
                tobz.reply(from, mess.wait, id)
                const lpornhub1 = argz[1]
                const lpornhub22 = argz[2]
                const lpornhub23 = argz[3]
                if (lpornhub1.length > 10) return tobz.reply(from, '*Teks1 Terlalu Panjang!*\n_Maksimal 10 huruf!_', id)
                if (lpornhub22.length > 30) return tobz.reply(from, '*Teks2 Terlalu Panjang!*\n_Maksimal 10 huruf!_', id)

                if (lpornhub23.length > 30) return tobz.reply(from, '*Teks3 Terlalu Panjang!*\n_Maksimal 10 huruf!_', id)                
                await tobz.sendFileFromUrl(from, `https://api.vhtear.com/googletext?text1=${lpornhub1}&text2=${lpornhub22}&text2=${lpornhub23}&apikey=${vhtearkey}`)
                await limitAdd(serial)
            } else {
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah */googlemaker [ |Teks1|Teks2|teks 3 ]*, *`, id)
            }
            break
        case prefix+'glitch':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Zfa|Dev DIRGA*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                tobz.reply(from, mess.wait, id)
                ...
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Zfa|Dev DIRGA*`, id)
            }
            await limitAdd(serial)
            tobz.reply(from, 'wa.me/628141419896@c.us https://www.instagram.com/dirgaap._ Owner BOT', id)
            break
            case prefix+'beritahoax':
					...
					data = await fetch(`https://api.vhtear.com/cerita_sex&apikey=${vhtearkey}`, {method: 'get'})
					teks = '=================\n'
        ...
        tobz.reply(from, '*WAJIB GUNAKAN GUILD KALIAN SEKARANG,INGAT GUILD KALIAN SEKARANG!!JIKA TIDAK RESIKO BANNED*', id)
                //argz = body.trim().split('|')
               if (argz.length >= 3) {
               ...
               // const guild = argz[3]
               // const bugi = body.slice(11)
           // if (args.length === 1) return tobz.reply(from, 'Daftar dengan benar*')
            if(!bugi) return
            if(isGroupMsg){
                tobz.sendText(ownerNumber, `*[JOGJA PENDAFTARAN]*\n*WAKTU* : ${time}\nNO PENGIRIM : wa.me/${sender.id.match(/\d+/g)}\nGroup : ${formattedTitle}\n\n${bugi}`)
                tobz.sendText(Admin1, `*[JOGJA PENDAFTARAN]*\n*WAKTU* : ${time}\nNO PENGIRIM : wa.me/${sender.id.match(/\d+/g)}\nGroup : ${formattedTitle}\n\n${bugi}`)
                tobz.reply(from, 'Thanks data udah terkirim.' ,id)
            }else{
                tobz.sendText(ownerNumber, `*[JOGJA PENDAFTARAN]*\n*WAKTU* : ${time}\nNO PENGIRIM : wa.me/${sender.id.match(/\d+/g)}\n\n${bugi}`)
                tobz.sendText(Admin1, `*[JOGJA PENDAFTARAN]*\n*WAKTU* : ${time}\nNO PENGIRIM : wa.me/${sender.id.match(/\d+/g)}\nGroup : ${formattedTitle}\n\n${bugi}`)
                tobz.reply(from, 'Thanks data udah terkirim jika ingin chat owner wa.me/628141419896@c.us.', id)
            }
            
                    //if(isNaN(umurnye)) return await tobz.reply(from, 'Umur harus berupa angka!!', id)
                    //if(umurnye >= 40) return await tobz.reply(from, 'Kamu terlalu tua, kembali lagi ke masa muda untuk menggunakan BOtT ', id)
                   // if(guild == undefined || guild == ' '|| guild == '|  ' )return tobz.reply(from, 'wajib gunakan guild',id)
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
                        ...
                        await tobz.reply(from, `Format yang kamu masukkan salah, kirim ${prefix}daftar |nama|umur|guild\n\ncontoh format: ${prefix}daftar |dirga|17|borocial jogja\n\ncukup gunakan nama depan/panggilan saja`, id) //if user is not registered
                    }
                break
            case prefix+'daftarulang':
                    if (!isAdmin) return tobz.reply(from, 'Command ini hanya dapat digunakan oleh admin DIRGA', id)  
...
${desc}`)
        ...
        case prefix+'creator':
            tobz.sendContact(chatId, `6282224077213@c.us`)
            tobz.reply(from, 'https://www.instagram.com/dirgaap._ Owner BOT', id)
            break
        ...
        case prefix+'nsfw':
            ...
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin DIRGA!', id)
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin group!', id)
        ...
        case prefix+'simi':
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin DIRGA!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
        ...
        case prefix+'group':
            ...
            if (!isBotGroupAdmins) return tobz.reply(from, 'Fitur ini hanya bisa di gunakan ketika dirga menjadi admin', id)
        ...
        case prefix+'loli':
            ...
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            const loli = await axios.get(`https://api.vhtear.com/cerita_sex&apikey=${vhtearkey}`)
        ...
        case prefix+'maluser':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perbaikan', id)
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
...
🌐️ *URL*: ${url}`
	...
	case prefix+'bitly':
        ...
        case prefix+'nhentai': // SEARCH NHENTAI
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/628141419896', id)
          break
        ...
        case prefix+'xnxx':
            ...
            try {
                tobz.reply(from, mess.wait, id)
                const resq = await axios.get('http://api.vhtear.com/xxxdownload?link='+ args[1] +'&apiKey='+ melodickey)
...
*Text : ${swrt}*`
        ...
        case prefix+'water':
                ...
                if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                const wtr = body.slice(7)
                if (!wtr) return tobz.reply(from, 'Kirim perintah */water [teks]*\n\nContoh */water*', id)
                if (wtr.length > 7) return tobz.reply(from, 'Maksimal 7 Huruf!', id)
                tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                await tobz.sendFileFromUrl(from, `https://api.vhtear.com/water_maker?text=${wtr}&apikey=${vhtearkey}`,`${wtr}.jpg`,`water ${wtr}`, id)        
                await limitAdd(serial)
                break
        case prefix+'fire':
                    if(isReg(obj)) return
                    ...
                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                    const fire = body.slice(5)
                    if (!fire) return tobz.reply(from, 'Kirim perintah */fire [teks]*\n\nContoh */fire*', id)
                    if (fire.length > 7) return tobz.reply(from, 'Maksimal 7 Huruf!', id)
                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/fire_maker?text=${fire}&apikey=${vhtearkey}`,`${fire}.jpg`,`fire ${fire}`, id)        
                    await limitAdd(serial)
                    break
         case prefix+'metal':
                        if(isReg(obj)) return
                        ...
                        if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                        if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                        const met = body.slice(6)
                        if (!met) return tobz.reply(from, 'Kirim perintah */metal [teks]*\n\nContoh */metal*', id)
                        if (met.length > 7) return tobz.reply(from, 'Maksimal 7 Huruf!', id)
                        tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                        await tobz.sendFileFromUrl(from, `https://api.vhtear.com/metal_maker?text=${met}&apikey=${vhtearkey}`,`${met}.jpg`,`metal ${met}`, id)        
                        await limitAdd(serial)
                        break
                        case prefix+'ballonlove':
                            if(isReg(obj)) return
                            if(cekumur(cekage)) return
                            
                            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                            if (args.length === 1) return tobz.reply(from, `Kirim perintah */ballonlove [ |Teks1|Teks2 ]*, contoh */ballonlove |Zfa| Dirga*`, id)
                            argz = body.trim().split('|')
                            if (argz.length >= 2) {
                                tobz.reply(from, mess.wait, id)
                                const lbal = argz[1]
                                const lbal2 = argz[2]
                                if (lbal.length > 9) return tobz.reply(from, '*Teks1 Terlalu Panjang!*\n_Maksimal 9 huruf!_', id)
                                if (lbal2.length > 9) return tobz.reply(from, '*Teks2 Terlalu Panjang!*\n_Maksimal 9 huruf!_', id)
                                tobz.sendFileFromUrl(from, `https://api.vhtear.com/balloonmaker?text1=${lbal}&text2=${lbal2}&apikey=${vhtearkey}`)
                                await limitAdd(serial)
                            } else {
                                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah */ballonlove [ |Teks1|Teks2 ]*, contoh */ballonlove |Zfa|Dev DIRGA*`, id)
                            }
                            break
                            case prefix+'lock':
                                if(isReg(obj)) return
                                ...
                                if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                if (args.length === 1) return tobz.reply(from, `Kirim perintah */lock [ |Teks1|Teks2 ]*, contoh */lock |Zfa| Dirga*`, id)
                                argz = body.trim().split('|')
                                if (argz.length >= 2) {
                                    tobz.reply(from, mess.wait, id)
                                    const lpad = argz[1]
                                    const lpad2 = argz[2]
                                    if (lpad.length > 12) return tobz.reply(from, '*Teks1 Terlalu Panjang!*\n_Maksimal 6 huruf!_', id)
                                    if (lpad2.length > 20) return tobz.reply(from, '*Teks2 Terlalu Panjang!*\n_Maksimal 6 huruf!_', id)
                                    tobz.sendFileFromUrl(from, `https://api.vhtear.com/padlock?text1=${lpad}&text2=${lpad2}&apikey=${vhtearkey}`)
                                    await limitAdd(serial)
                                } else {
                                    await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah */ballonlove [ |Teks1|Teks2 ]*, contoh */ballonlove |Zfa|Dev DIRGA*`, id)
                                }
                                ...
                                if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                if (args.length === 1) return tobz.reply(from, `Kirim perintah */ffbanner [ |Teks1|Teks2 ]*, contoh */ffbanner |Zfa| Dirga*`, id)
                                argz = body.trim().split('|')
                                if (argz.length >= 2) {
                                    tobz.reply(from, mess.wait, id)
                                    const lff = argz[1]
                                    const lff2 = argz[2]
                                    if (lff.length > 7) return tobz.reply(from, '*Teks1 Terlalu Panjang!*\n_Maksimal 7 huruf!_', id)
                                    if (lff2.length > 7) return tobz.reply(from, '*Teks2 Terlalu Panjang!*\n_Maksimal 7 huruf!_', id)
                                    tobz.sendFileFromUrl(from, `https://api.vhtear.com/bannerff?title=${lff}&text=${lff2}&apikey=${vhtearkey}`)
                                    await limitAdd(serial)
                                } else {
                                    await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah */ffbanner [ |Teks1|Teks2 ]*, contoh */ffbanner|Zfa|Dev DIRGA*`, id)
                                }
                                break
                                case prefix+'ffalok':
                                    if(isReg(obj)) return
                                    ...
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const fireal = body.slice(7)
                                    if (!fireal) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (fireal.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=alok&text=${fireal}&apikey=${vhtearkey}`,`${fireal}.jpg`,`ffalok ${fireal}`, id)        
                                    await limitAdd(serial)
                                    break
                                case prefix+'gamelogo':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    tobz.reply(from, 'Logo ini random', id)
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const fireald = body.slice(9)
                                    if (!fireald) return tobz.reply(from, 'Kirim perintah */gamelogo [teks]*\n\nContoh */gamelogo*', id)
                                    if (fireald.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar ajg..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/gamelogo?text=${fireald}&apikey=${vhtearkey}`,`${fireald}.jpg`,`logo ${fireald}`, id)        
                                    await limitAdd(serial)
                                ...
                                case prefix+'galaxy':
                                    ...
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealdq = body.slice(8)
                                    if (!firealdq) return tobz.reply(from, 'Kirim perintah */galaxy [teks]*\n\nContoh */gamelogo*', id)
                                    if (firealdq.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/galaxytext?text=${firealdq}&apikey=${vhtearkey}`,`${firealdq}.jpg`,` ${firealdq}`, id)        
                                    await limitAdd(serial)
                                    break
                                case prefix+'slidingtext':
                                    ...
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealdq1 = body.slice(12)
                                    if (!firealdq1) return tobz.reply(from, 'Kirim perintah */galaxy [teks]*\n\nContoh */gamelogo*', id)
                                    if (firealdq1.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/slidingtext?text=${firealdq1}&apikey=${vhtearkey}`,`${firealdq1}.jpg`,` ${firealdq1}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'textgif':
                                    if(isReg(obj)) return
                                    ...
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealdq1v = body.slice(12)
                                    if (!firealdq1v) return tobz.reply(from, 'Kirim perintah */textgif [teks]*\n\nContoh', id)
                                    if (firealdq1v.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendStickerfromUrl(from, `https://api.vhtear.com/textgif?text=${firealdq1v}&apikey=${vhtearkey}`,`${firealdq1v}.jpg`,` ${firealdq1v}`, id)        
                                    await limitAdd(serial)
                                    break
                                case prefix+'ffkelly':
                                    if(isReg(obj)) return
                                    ...
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const fireal1 = body.slice(8)
                                    if (!fireal1) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (fireal1.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=kelly&text=${fireal1}&apikey=${vhtearkey}`,`${fireal1}.jpg`,`ff ${fireal1}`, id)        
                                    await limitAdd(serial)
                                    break
                                case prefix+'ffandrew':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const fireal2 = body.slice(7)
                                    if (!fireal2) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (fireal2.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=andrew&text=${fireal2}&apikey=${vhtearkey}`,`${fireal2}.jpg`,`ff ${fireal2}`, id)        
                                    await limitAdd(serial)
                                    break
                                    
                                    case prefix+'ffhayato':
                                        if(isReg(obj)) return
                                        if(cekumur(cekage)) return
                                        if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                        if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                        const fireal3 = body.slice(9)
                                        if (!fireal3) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                        if (fireal3.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                        tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                        await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=hayato&text=${fireal3}&apikey=${vhtearkey}`,`${fireal3}.jpg`,`ff ${fireal3}`, id)        
                                        await limitAdd(serial)
                                        break
                                        case prefix+'ffantonio':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const fireall = body.slice(10)
                                    if (!fireall) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (fireall.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=antonio&text=${fireall}&apikey=${vhtearkey}`,`${fireall}.jpg`,`ff ${fireall}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffcaroline':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealq = body.slice(12)
                                    if (!firealq) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (firealq.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=caroline&text=${firealq}&apikey=${vhtearkey}`,`${firealq}.jpg`,`ff ${firealq}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffford':
                                    if(isReg(obj)) return
                                    ...
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const ford = body.slice(7)
                                    if (!ford) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (ford.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=ford&text=${ford}&apikey=${vhtearkey}`,`${ford}.jpg`,`ff ${ford}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffkla':
                                    if(isReg(obj)) return
                                    ...
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealw = body.slice(7)
                                    if (!firealw) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (firealw.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=kla&text=${firealw}&apikey=${vhtearkey}`,`${firealw}.jpg`,`ff ${firealw}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'fflaura':
                                    if(isReg(obj)) return
                                    ...
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const fireals = body.slice(8)
                                    if (!fireals) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (fireals.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=laura&text=${fireals}&apikey=${vhtearkey}`,`${fireals}.jpg`,`ff ${fireals}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffmaxim':
                                    if(isReg(obj)) return
                                    ...
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealx = body.slice(7)
                                    if (!firealx) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (firealx.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=maxim&text=${firealx}&apikey=${vhtearkey}`,`${firealx}.jpg`,`ff ${firealx}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffmiguel':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealz = body.slice(7)
                                    if (!firealz) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (firealz.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=miguel&text=${firealz}&apikey=${vhtearkey}`,`${fireal1}.jpg`,`ff ${firealz}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffmoco':
                                    if(isReg(obj)) return
                                    ...
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealr = body.slice(7)
                                    if (!firealr) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (firealr.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=moco&text=${firealr}&apikey=${vhtearkey}`,`${firealr}.jpg`,`ff ${firealr}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffnikita':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealc = body.slice(7)
                                    if (!firealc) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (firealc.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=nikita&text=${firealc}&apikey=${vhtearkey}`,`${firealc}.jpg`,`ff ${firealc}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffolivia':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealv = body.slice(7)
                                    if (!firealv) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (firealv.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=olivia&text=${firealv}&apikey=${vhtearkey}`,`${firealv}.jpg`,`ff ${firealv}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffpaloma':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealf = body.slice(7)
                                    if (!firealf) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (firealf.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=paloma&text=${firealf}&apikey=${vhtearkey}`,`${firealf}.jpg`,`ff ${firealf}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffrafael':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firealk = body.slice(7)
                                    if (!firealk) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (firealk.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=rafael&text=${firealk}&apikey=${vhtearkey}`,`${firealk}.jpg`,`ff ${firealk}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffwukong':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const firea = body.slice(9)
                                    if (!firea) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (firea.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=wukong&text=${firea}&apikey=${vhtearkey}`,`${firea}.jpg`,`ff ${firea}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffshani':
                                    if(isReg(obj)) return
                                    ...
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const fir = body.slice(7)
                                    if (!fir) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (fir.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=shani&text=${fir}&apikey=${vhtearkey}`,`${fir}.jpg`,`ff ${fir}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffnotora':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const fireal12 = body.slice(7)
                                    if (!fireal12) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (fireal12.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=notora&text=${fireal12}&apikey=${vhtearkey}`,`${fireal12}.jpg`,`ff ${fireal12}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffjoseph':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const fireal13 = body.slice(7)
                                    if (!fireal13) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (fireal13.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=joseph&text=${fireal13}&apikey=${vhtearkey}`,`${fireal13}.jpg`,`ff ${fireal13}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffalvaro':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const fireal14 = body.slice(7)
                                    if (!fireal14) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (fireal14.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=alvaro&text=${fireal14}&apikey=${vhtearkey}`,`${fireal14}.jpg`,`ff ${fireal14}`, id)        
                                    await limitAdd(serial)
                                    break
                                    case prefix+'ffsteffie':
                                    if(isReg(obj)) return
                                    if(cekumur(cekage)) return
                                    if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                                    if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
                                    const fireal16 = body.slice(7)
                                    if (!fireal16) return tobz.reply(from, 'Kirim perintah */ffalok [teks]*\n\nContoh */ffalok*', id)
                                    if (fireal16.length > 20) return tobz.reply(from, 'Maksimal 10 Huruf!', id)
                                    tobz.sendText(from, '_Sedang diproses, mohon tunggu sebentar!..._', id)
                                    await tobz.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=steffie&text=${fireal16}&apikey=${vhtearkey}`,`${fireal16}.jpg`,`ff ${fireal16}`, id)        
                                    await limitAdd(serial)
                                    break
...
➸ *Cara:* ${cara}`
        ...
        case prefix+'cersex':
            ...
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            try {
            const resps = await axios.get('https://api.vhtear.com/cerita_sex&apikey=' + vhtearkey)
            const { cerita, image, judul  } = resps.data.result
            const rmk3 = `*Cerita Ditemukan!*
➸ *Judul:* ${judul}
➸ *Url:* ${image}
➸ *Cerita:* ${cerita}`
            ...
            const pictk = await bent("buffer")(image)
            const base64 = `data:image/jpg;base64,${pictk.toString("base64")}`
            tobz.sendImage(from, base64, judul, rmk3)
            await limitAdd(serial)
            } catch (err) {
             console.error(err.message)
             await tobz.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, Resep tidak ditemukan')
...
• *Link:* https://twitter.com/${user.screen_name}`
          ...
          case prefix+'igstalk':
            ...
            const { username, biography, follow, follower, full_name, picture, post_count, is_private } = istalk2.data.result
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'addbadword':
            ...
            const wordi = body.slice(12)
            var cek = dbbadword.includes(wordi);
            if(cek){
                return tobz.reply(from, `Badword Sudah Ada Di Database`, id)
            } else { 
                dbbadword.push(wordi)
        ...
        case prefix+'listbadword':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin BOT!`, id)
                ...
                let nomre = 1
                    ...
                    break
        //list permulaan
        case prefix+'addblacklist':
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin BOT!`, id)
            if (!args.length >= 1) return tobz.reply(from, `Masukkan member yang akan di blacklist `, id) 
            const blacki = body.slice(12)
            var cek = dbblacklist.includes(blacki);
            if(cek){
                return tobz.reply(from, `Badword Sudah Ada Di Database`, id)
            } else { 
                dbblacklist.push(blacki)
                fs.writeFileSync('./lib/database/dbblacklist.json', JSON.stringify(dbblacklist))
                tobz.reply(from, `Success Menambahkan Blacklist member\nTotal Data Blacklist Sekarang : *${dbblacklist.length - 1}*`, id)
            }
            break
        case prefix+'delblacklist':
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Owner BOT!`, id)
                const delbl = dbblacklist.indexOf(body.slice(12))
                dbblacklist.splice(delbl, 1)
                fs.writeFileSync('./lib/database/dbblacklist.json', JSON.stringify(dbblacklist))
                tobz.reply(from, `Success Menghapus Blacklist!`, id)
            break
        case prefix+'listblacklist':
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin BOT!`, id)
                const black = fs.readFileSync('./lib/database/dbblacklist.json')
                const listblack = JSON.parse(black)
                let listb = '*「 LIST BLACKLIST 」*\n'
                listb += `*Total : ${listblack.length}*\n`
                let nomrex = 1
                     for (let i = 0; i < listblack.length; i++){
                        listb += `\n*${nomrex}.* ${listblack[i]}`
                        nomrex++
                    }
                    tobz.sendText(from, listb) 
                    break
                    //
                     //
            case prefix+'addbl':{
                if (!args.length >= 1) return tobz.reply(from, 'BACOTAN NYA MANA ANJING?? DASAR BODOH!', id)  
                    const listbc = body.slice(10)
                    dbcot.push(listbc)
                    fs.writeFileSync('./lib/database/dbblacklist.json', JSON.stringify(dbcot))
                    tobz.reply(from, `Sukses menambahkan Kata bacot ke database\nTotal data bacot sekarang : *${dbcot.length - 1}*`, id)
                }
                break
            case prefix+ 'delbl':
                    if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan didalam grup!`, id)
                        const delbdc = dbcot.indexOf(body.slice(12))
                        dbcot.splice(delbdc, 1)
                        fs.writeFileSync('./lib/database/dbblacklist.json', JSON.stringify(dbcot))
                        tobz.reply(from, `Success Menghapus Bacot!`, id)
                    break
                    case prefix+'listbl':
                            const bacul = dbcot
        ...
        case prefix+'ekickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner BOT', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika dirga menjadi admin', id)
            const allMem = await tobz.getGroupMembers(groupId)
            for (let i = 0; i < allMem.length; i++) {
                if (ownerNumber.includes(allMem[i].id)) {
                    console.log('Maaf dirga Ga Bisa Kick Admin Grub,dirga Takut Dimarahin')
                } else {
        ...
        case prefix+'oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin BOT', id)
            await tobz.sendText(from,' DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group,Jadi member jan sok keras!!', id)
        ...
        case prefix+'delete':
            ...
            if (!isGroupAdmins) return tobz.reply(from, 'Fitur ini hanya bisa di gunakan oleh admin group', id)
            if (!quotedMsg) return tobz.reply(from, 'Salah!!, kirim perintah *#delete [tagpesanbot]*', id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, 'Maaf kak!!, dirga tidak bisa mengahpus chat user lain!', id)
            tobz.deleteMessage(quotedMsgObj.chatId, quotedMsgObj.id, false)
            break
        case prefix+'sider':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)                
            if (!quotedMsg) return tobz.reply(from, `Tolong Reply Pesan `, id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, `Tolong Reply Pesan `, id)
            try {
                ...
                tobz.sendTextWithMentions(from, `DIH, Ngeread PARAH KICK AJA BEBAN NTAR WKWK CANDA BEBAN \n${list}`)
            } catch(err) {
                console.log(err)
                tobz.reply(from, `Maaf, Belum Ada Yang Membaca Pesan  atau Mereka Menonaktifkan Read Receipts`, id)    
            }
            break
        ...
        case prefix+'getses':
            ...
            break
        case prefix+'dirgaadmin':
            let admn = `Ini adalah daftar admin BOT\nTotal : ${adminNumber.length}\n`
...
• Limit: ${limitCount-updated.limit}`
        ...
        case prefix+'addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner BOT!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin BOT!', id)
                }
            break
        case prefix+'deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner BOT!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin BOT!', id)
            break
        case prefix+'block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner BOT!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case prefix+'ban':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh owner BOT!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin BOT`, id)
        ...
        case prefix+'botgroup':
            tobz.reply(from, `Link Group Bot:  ${pushname}`, id)
            break
...
// By Gimenz
        ...
        default:
            ...
            if (command.startsWith('#')) {
                tobz.reply(from, `Maaf ${pushname}, Command *${args[0]}* Tidak Terdaftar Di Dalam */menu* Ketik /menu untuk melihat daftar`, id)
            }
            await tobz.sendSeen(from) 
            }
        }
